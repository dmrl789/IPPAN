# Clean final solution: Open UI and provide clear instructions
...
Write-Host "`nSTEP-BY-STEP INSTRUCTIONS:" -ForegroundColor Green
...
Write-Host "`nCURRENT BLOCKCHAIN STATUS:" -ForegroundColor Cyan
try {
    ...
    if ($state.latest_block_height -gt 0) {
        Write-Host "`nSUCCESS: Blockchain is building with height $($state.latest_block_height)!" -ForegroundColor Green
