...
async function apiGetWallet(address: string): Promise<WalletState> {
  const main: Asset = { symbol: "IPN", name: "IPPAN", balance: Math.round(Math.random() * 10_000) / 100 };
  const tokens: Asset[] = [
    main,
    { symbol: "STAKE", name: "Staked IPN (sIPN)", balance: Math.round(Math.random() * 5000) / 100 },
  ];
  const acts: Activity[] = [
          { id: crypto.randomUUID(), title: "Sent Payment", subtitle: "To: iCzfRuaeBuDyHYkzP2PO6zC9d454cfc419a4bcdf4c3d686cc34a0b64b7ed4X", delta: -50, when: nowMinus(2) },
    { id: crypto.randomUUID(), title: "Staking Reward", subtitle: "Validator rewards", delta: +12.5, when: nowMinus(24) },
    { id: crypto.randomUUID(), title: "Domain Registration", subtitle: "example.ippan", delta: -100, when: nowMinus(72) },
  ];
  const staking: Staking = { staked: 2500, rewards: 42.75 };
  const domains = ["example.ippan", "alpha.dns"];
  return new Promise((r) => setTimeout(() => r({ address, assets: tokens, activities: acts, staking, domains }), 250));
}
