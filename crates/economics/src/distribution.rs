...
pub fn distribute_round(
    ...
    // Apply fee cap
    let (capped_fees_micro, fee_cap_applied_micro) = apply_fee_cap(fees_micro, emission_micro, params);
...
fn apply_fee_cap(
    ...
    let max_fees_micro = (emission_micro * params.fee_cap_numer as u128) / params.fee_cap_denom as u128;
...
pub fn calculate_validator_total_rewards(validator_id: &[u8; 32], all_payouts: &[Payouts]) -> MicroIPN {
    all_payouts
        .iter()
...
pub fn validate_participation_set(participants: &ParticipationSet) -> Result<()> {
    ...
    for participant in participants {
        ...
        if participant.reputation_score < 0.0 || participant.reputation_score > 2.0 {
            return Err(anyhow!("Invalid reputation score: {}", participant.reputation_score));
        }
    }
