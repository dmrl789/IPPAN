...
impl RewardSink {
    ...
    pub fn credit_round_payouts(&mut self, round: u64, payouts: &Payouts) -> Result<()> {
        ...
        info!(
            target: "treasury",
            "Round {}: Credited {} micro-IPN across {} validators",
            round,
            round_total,
            payouts.len()
        );
    ...
    pub fn settle_to_accounts(&self, accounts: &mut dyn AccountLedger) -> Result<()> {
        ...
        for (round, payouts) in &self.rounds {
            for (vid, amount) in payouts {
                ...
                debug!(
                    target: "treasury",
                    "Settled {} micro-IPN to validator {:?} for round {}",
                    amount,
                    vid,
                    round
                );
        ...
        info!(
            target: "treasury",
            "Settled {} micro-IPN across {} rounds to accounts",
            total_settled,
            rounds_settled
        );
    ...
    pub fn clear_settled_payouts(&mut self, up_to_round: u64) {
        let rounds_to_remove: Vec<u64> = self.rounds
            .keys()
    ...
    pub fn get_statistics(&self) -> RewardPoolStatistics {
        ...
        let total_validators: usize = self.rounds
            .values()
            .flat_map(|p| p.keys())
...
/// Statistics about the reward pool
...
/// Reward pool manager that coordinates between emission and distribution
...
impl RewardPoolManager {
    ...
    pub fn process_round_rewards(&mut self, round: u64, payouts: &Payouts) -> Result<()> {
        ...
        // Immediately settle to accounts
    ...
    /// Get mutable reward sink for direct manipulation
    pub fn get_sink_mut(&mut self) -> &mut RewardSink {
        &mut self.sink
    }

    /// Get account ledger for balance queries
    pub fn get_account_ledger(&self) -> &dyn AccountLedger {
        self.account_ledger.as_ref()
    }

    /// Get account ledger for balance updates
...
mod tests {
    ...
    fn test_empty_payouts() {
        let mut sink = RewardSink::new();
        let payouts = HashMap::new();
        
        sink.credit_round_payouts(1, &payouts).unwrap();
        
        assert_eq!(sink.get_total_distributed(), 0);
        assert_eq!(sink.get_rounds().len(), 0);
    }
