...
impl MockRpcClient {
    fn new() -> Self {
        ...
        balances.insert(
            "i1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef".to_string(),
            5000,
        );
        balances.insert(
            "iabcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890".to_string(),
            3000,
        );
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    wallet.create_wallet(
        "Advanced IPPAN Wallet".to_string(),
    ...
    println!(
        "✅ Created backup 1: {}",
        backup1.file_name().unwrap().to_string_lossy()
    );

    // Simulate some changes
    let temp_address =
        wallet.generate_address(Some("Temporary".to_string()), Some("secure_password_123"))?;
    println!("   Added temporary address: {}", temp_address);

    let backup2 = wallet.create_backup()?;
    println!(
        "✅ Created backup 2: {}",
        backup2.file_name().unwrap().to_string_lossy()
    );
    ...
    println!(
        "✅ Wallet exported with {} addresses",
        export_data.wallet_state.addresses.len()
    );
    println!("   Checksum: {}", export_data.checksum);
    println!(
        "   Export time: {}",
        export_data.created_at.format("%Y-%m-%d %H:%M:%S")
    );
