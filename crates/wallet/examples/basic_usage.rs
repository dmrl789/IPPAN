...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Wallet exported ({} addresses)", backup_data.wallet_state.addresses.len());
    ...
    println!("   New label: {}", addr_info.label.as_deref().unwrap_or("No label"));
    ...
    if addresses.len() >= 2 {
        ...
        println!("   Simulating transaction from {} to {}", from_addr, to_addr);
