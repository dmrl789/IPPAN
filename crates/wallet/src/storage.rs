...
impl WalletStorage {
    ...
    pub fn save_wallet(&self) -> Result<()> {
        ...
        if let Some(ref wallet_state) = *state {
            let data = serde_json::to_string_pretty(wallet_state)
                .map_err(|e| WalletError::StorageError(format!("Failed to serialize wallet: {}", e)))?;
            ...
            fs::rename(&temp_path, &self.wallet_path)
                .map_err(|e| WalletError::StorageError(format!("Failed to rename wallet file: {}", e)))?;
        }
    ...
    pub fn get_wallet_state(&self) -> Result<WalletState> {
        let state = self.wallet_state.read();
        state.clone().ok_or_else(|| WalletError::WalletNotInitialized)
    }
    ...
    pub fn create_backup(&self) -> Result<PathBuf> {
        ...
        let backup_file = self.backup_path.join(format!("wallet_backup_{}.json", timestamp));
    ...
    pub fn restore_from_backup(&self, backup_path: &Path, password: Option<&str>) -> Result<()> {
        ...
        if !backup.verify_checksum() {
            return Err(WalletError::StorageError("Backup checksum verification failed".to_string()));
        }
    ...
    pub fn list_backups(&self) -> Result<Vec<PathBuf>> {
        ...
        if self.backup_path.exists() {
            let entries = fs::read_dir(&self.backup_path)
                .map_err(|e| WalletError::StorageError(format!("Failed to read backup directory: {}", e)))?;
            
            for entry in entries {
                let entry = entry.map_err(|e| WalletError::StorageError(format!("Failed to read backup entry: {}", e)))?;
                let path = entry.path();
    ...
    pub fn import_wallet(&self, backup: WalletBackup, password: Option<&str>) -> Result<()> {
        if !backup.verify_checksum() {
            return Err(WalletError::StorageError("Import data checksum verification failed".to_string()));
        }
...
mod tests {
    ...
    fn test_wallet_initialization() {
        ...
        storage.initialize("Test Wallet".to_string(), Some("password123")).unwrap();
        assert!(storage.wallet_exists());
    }
    ...
    fn test_wallet_backup_restore() {
        ...
        // Restore from backup
        storage.restore_from_backup(&backup_path, Some("password123")).unwrap();
