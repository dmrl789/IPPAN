...
impl CliApp {
    ...
    pub async fn run(&mut self, cli: Cli) -> Result<()> {
        match cli.command {
            Commands::Init { name, password } => {
                self.init_wallet(name, password).await
            },
            ...
            Commands::Restore { backup_path } => {
                self.restore_wallet(backup_path).await
            },
            ...
            Commands::Import { input } => {
                self.import_wallet(input).await
            },
        }
    }
    ...
    async fn load_wallet(&mut self) -> Result<()> {
        ...
        let password = self.prompt_password_optional("Enter wallet password (or press Enter for no password): ")?;
        let wallet_manager = WalletManager::new(storage, None);
    ...
    async fn generate_addresses(&self, label: Option<String>, count: usize) -> Result<()> {
        ...
        if count == 1 {
            ...
            let addresses = wallet.generate_addresses(count, Some(label_prefix), password.as_deref())?;
    ...
    async fn show_history(&self, address: Option<String>) -> Result<()> {
        ...
        println!("📜 Transaction History ({} transactions):", transactions.len());
        println!("{:<20} {:<65} {:<65} {:<15} {:<10}", "Time", "From", "To", "Amount", "Status");
        println!("{}", "-".repeat(200));
        
        for tx in transactions.iter().take(20) { // Show last 20 transactions
    ...
    async fn restore_wallet(&mut self, backup_path: PathBuf) -> Result<()> {
        ...
        println!("✅ Wallet restored successfully from: {}", backup_path.display());
