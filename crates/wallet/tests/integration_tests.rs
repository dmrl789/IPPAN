...
async fn test_wallet_lifecycle() {
    ...
    let address1 = wallet.generate_address(Some("Test Address 1".to_string()), Some("password123")).unwrap();
    let address2 = wallet.generate_address(Some("Test Address 2".to_string()), Some("password123")).unwrap();
    ...
    let tx_hash = wallet.send_transaction(&address1, &address2, 100, Some("password123")).unwrap();
...
async fn test_multiple_address_generation() {
    ...
    let addresses = wallet.generate_addresses(10, Some("Batch".to_string()), Some("password123")).unwrap();
...
async fn test_wallet_backup_restore() {
    ...
    wallet.create_wallet("Test Wallet".to_string(), Some("password123")).unwrap();
    let address1 = wallet.generate_address(Some("Address 1".to_string()), Some("password123")).unwrap();
    let address2 = wallet.generate_address(Some("Address 2".to_string()), Some("password123")).unwrap();
    ...
    wallet2.restore_from_backup(&backup_path, Some("password123")).unwrap();
...
async fn test_wallet_encryption() {
    ...
    wallet.create_wallet("Encrypted Wallet".to_string(), Some("secure_password")).unwrap();
    
    // Generate address with password
    let address = wallet.generate_address(Some("Encrypted Address".to_string()), Some("secure_password")).unwrap();
...
async fn test_address_management() {
    ...
    // Test label update
    wallet.update_address_label(&address1, Some("Updated Label".to_string())).unwrap();
...
async fn test_transaction_history() {
    ...
    let address1 = wallet.generate_address(Some("Sender".to_string()), Some("password123")).unwrap();
    let address2 = wallet.generate_address(Some("Receiver".to_string()), Some("password123")).unwrap();
...
async fn test_wallet_export_import() {
    ...
    wallet.create_wallet("Export Test Wallet".to_string(), Some("password123")).unwrap();
...
async fn test_error_handling() {
    ...
    let result = wallet.send_transaction("invalid_address", "another_invalid", 100, Some("password123"));
    assert!(result.is_err());
    
    // Test sending with insufficient balance
    let address = wallet.generate_address(None, Some("password123")).unwrap();
    let result = wallet.send_transaction(&address, "i1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef", 2000, Some("password123"));
    assert!(result.is_err());
}