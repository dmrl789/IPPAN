...
pub struct ModelMetadata {
    ...
    /// SHA-256 hash of the model weights/structure
...
/// Compute SHA-256 hash of model structure
fn compute_model_hash(model: &crate::gbdt::GBDTModel) -> [u8; MODEL_HASH_SIZE] {
    // Serialize model to canonical JSON
    let json = serde_json::to_string(model).expect("model serialization should not fail");
