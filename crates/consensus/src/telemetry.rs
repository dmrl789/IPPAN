...
impl TelemetryManager {
    ...
    pub fn load_from_storage(&self) -> Result<()> {
        ...
        *cache = all_telemetry;
        debug!("Loaded {} validator telemetry records from storage", cache.len());
        Ok(())
    }
    ...
    pub fn record_block_proposal(&self, validator_id: &[u8; 32]) -> Result<()> {
        ...
        let telemetry = cache.entry(*validator_id).or_insert_with(|| ValidatorTelemetry {
        ...
        // Persist to storage
        self.storage.store_validator_telemetry(validator_id, telemetry)?;
        
        debug!("Recorded block proposal for validator {}", hex::encode(validator_id));
        Ok(())
    }
    ...
    pub fn advance_round(&self) -> Result<()> {
        ...
        *current_round += 1;
        ...
        for (validator_id, telemetry) in cache.iter_mut() {
            ...
            // Persist updates
            if let Err(e) = self.storage.store_validator_telemetry(validator_id, telemetry) {
                warn!("Failed to persist telemetry for validator {}: {}", 
                      hex::encode(validator_id), e);
            }
        }
        
        debug!("Advanced to round {}, updated {} validator telemetry records", round, cache.len());
        Ok(())
    }
    ...
    pub fn record_slash(&self, validator_id: &[u8; 32]) -> Result<()> {
        ...
        if let Some(telemetry) = cache.get_mut(validator_id) {
            ...
            self.storage.store_validator_telemetry(validator_id, telemetry)?;
            warn!("Recorded slash for validator {}, total slashes: {}", 
                  hex::encode(validator_id), telemetry.slash_count);
        }
        Ok(())
    }
}
