...
use ippan_storage::{
    Account, ChainState, MemoryStorage, SledStorage, Storage, ValidatorTelemetry,
};
use ippan_types::{
...
fn create_test_transaction(from: [u8; 32], to: [u8; 32], amount_val: u64, nonce: u64) -> Transaction {
...
fn test_validator_telemetry_storage<S: Storage>(storage: &S) {
    ...
    storage.store_validator_telemetry(&val1_id, &tel1_updated).unwrap();
...
fn test_multiple_blocks_same_storage() {
    ...
    for i in 0..10 {
        let parents = if i > 0 { vec![[i as u8 - 1; 32]] } else { vec![] };
