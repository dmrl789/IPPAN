...
fn generate_keypair(output: &PathBuf, name: &str, stdout: bool) -> Result<()> {
    ...
    if stdout {
        ...
        // Write private key
        fs::write(&private_path, &private_hex)
    ...
    println!("\n📋 Validator ID: {}", hex::encode(validator_id.as_bytes()));
...
fn derive_public_key(private_key_path: &PathBuf) -> Result<()> {
    ...
    if private_bytes.len() != 32 {
        anyhow::bail!("Invalid private key length: expected 32 bytes, got {}", private_bytes.len());
    }
...
fn verify_keypair(private_key_path: &PathBuf, public_key_path: &PathBuf) -> Result<()> {
    ...
    let public_bytes = hex::decode(public_hex)
        .context("Invalid hex format in public key file")?;
    ...
    if public_bytes.len() != 32 {
        anyhow::bail!("Invalid public key length: expected 32 bytes, got {}", public_bytes.len());
    }
    ...
    let provided_public = VerifyingKey::from_bytes(&public_key_bytes)
...
fn compute_validator_id(public_key_input: &str) -> Result<()> {
    ...
    let public_hex = if PathBuf::from(public_key_input).exists() {
        fs::read_to_string(public_key_input)
            .context("Failed to read public key file")?
    } else {
