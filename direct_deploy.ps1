# Direct Deploy IPPAN - Try Multiple Approaches
Import-Module Posh-SSH -ErrorAction SilentlyContinue

$SERVER1_IP = "188.245.97.41"
$SERVER2_IP = "135.181.145.174"

Write-Host "=== DIRECT DEPLOY IPPAN ===" -ForegroundColor Cyan

# Function to try connecting with multiple methods
function Try-Connect {
    param($ServerIP, $ServerName)
    
    Write-Host "Attempting to connect to $ServerName ($ServerIP)..." -ForegroundColor Green
    
    # Try different connection methods
    $methods = @(
        @{Type="Password"; Credential=@{Username="root"; Password="VgRfqNg3T4sx"}},
        @{Type="Password"; Credential=@{Username="root"; Password="root123"}},
        @{Type="Password"; Credential=@{Username="root"; Password="rescue123"}},
        @{Type="Password"; Credential=@{Username="root"; Password="hetzner123"}},
        @{Type="Key"; KeyPath="$env:USERPROFILE\.ssh\id_rsa_ippan"}
    )
    
    foreach ($method in $methods) {
        Write-Host "Trying $($method.Type) authentication..." -ForegroundColor Yellow
        try {
            if ($method.Type -eq "Password") {
                $securePassword = ConvertTo-SecureString $method.Credential.Password -AsPlainText -Force
                $credential = New-Object System.Management.Automation.PSCredential($method.Credential.Username, $securePassword)
                $session = New-SSHSession -ComputerName $ServerIP -Credential $credential -AcceptKey -ConnectionTimeout 10
            } else {
                $session = New-SSHSession -ComputerName $ServerIP -KeyFile $method.KeyPath -AcceptKey -ConnectionTimeout 10
            }
            
            if ($session) {
                Write-Host "✅ Connected to $ServerName using $($method.Type)!" -ForegroundColor Green
                return $session
            }
        } catch {
            Write-Host "Failed with $($method.Type): $($_.Exception.Message)" -ForegroundColor Red
        }
    }
...
# Try to connect and deploy to Server 1
Write-Host "=== DEPLOYING TO SERVER 1 ===" -ForegroundColor Cyan
$server1Session = Try-Connect -ServerIP $SERVER1_IP -ServerName "Server 1"
...
# Try to connect and deploy to Server 2
Write-Host ""
Write-Host "=== DEPLOYING TO SERVER 2 ===" -ForegroundColor Cyan
$server2Session = Try-Connect -ServerIP $SERVER2_IP -ServerName "Server 2"
