...
IPPAN now follows a **trunk-based development** workflow where all development occurs directly on the `master` branch. This document outlines the configuration, rationale, and guidelines for this approach.

## Branch Policy (MASTER ONLY)

- Work **ONLY** on the `master` branch.
- Do **NOT** create or use `main`, `develop`, `feature/*`, or any other branches for agent work.
...
- **CI/CD Triggers**: All workflows run on `master` branch only

### CI/CD Configuration

All GitHub Actions workflows have been configured to trigger exclusively on the `master` branch:
