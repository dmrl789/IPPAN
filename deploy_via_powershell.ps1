# Deploy IPPAN Services via PowerShell SSH
# This script will connect to both servers and deploy IPPAN services
...
Write-Host "=== IPPAN Deployment via PowerShell SSH ===" -ForegroundColor Cyan
Write-Host ""
...
function Deploy-IPPANOnServer {
    param([string]$ServerIP, [string]$ServerName, [string]$Username, [string]$Password)
...
# Add SSH public key (we'll get this from the local machine)
...
"@
            ...
            Write-Host "Running deployment commands on $ServerName..." -ForegroundColor Green
...
"@
            ...
            if ($ippanResult.ExitStatus -eq 0) {
                ...
                Write-Host "Output: $($ippanResult.Output)" -ForegroundColor Cyan
            } else {
...
# Deploy to Server 1
Deploy-IPPANOnServer -ServerIP $SERVER1_IP -ServerName "Server 1" -Username "root" -Password $RESCUE_PASSWORD
...
# Deploy to Server 2
Deploy-IPPANOnServer -ServerIP $SERVER2_IP -ServerName "Server 2" -Username "root" -Password $RESCUE_PASSWORD
