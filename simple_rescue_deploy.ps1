...
Write-Host "=== Simple Rescue Mode Deployment ===" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "=== Enabling Rescue Mode (Simple) ===" -ForegroundColor Green

# Enable rescue mode without SSH keys first
$rescueBody = @{
    rescue = "linux64"
} | ConvertTo-Json

# Server 1
Write-Host "Enabling rescue mode on Server 1..." -ForegroundColor Yellow
try {
...
# Server 2
Write-Host "Enabling rescue mode on Server 2..." -ForegroundColor Yellow
try {
