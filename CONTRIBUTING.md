...
| ---------------------- | --------------------- | -------------------------------------------------- |
| Main production branch | `main`                | Protected — only maintainers merge here.           |
...
1. **One PR per logical change** (max ≈ 500 LOC).
2. PR title = short imperative sentence.

   * ✅ `feat: add DAG-Fair emission reward calculator`
   * ❌ `added code changes`
3. Link related issue(s) using `Closes #123` or `Refs #456`.
4. Label the PR with:

   * `agent-alpha`, `agent-beta`, … or your human name
   * `crate:<crate-name>`
...
* Require:
  ...
  * ✅ All CI checks green
  * ✅ At least one review (human or verified orchestrator)
* **No fast-forward merges** — always **squash** to keep history linear.
...
* **GitHub Issues** → primary coordination
* **PR comments** → technical discussion only
* **/docs/roadmap.md** → tracks active milestones
...
1. 🧩 Commit small, merge fast.
2. 🧠 No crate-wide refactors without RFC.
3. 🧮 Keep all code deterministic and reproducible.
4. 🛑 Never merge if you don't understand the build impact.
5. ⚖️ The network's integrity > individual productivity.
